...
public class SearchControllerUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(search.length() == 0){
                valid = false;
                errorObject.setAvengerNameError("Please enter any character!");
            }
            if(valid){
                beans.setAvengerName(search);
                List<RegistrationDTO> result = beans.findLikeByAvengerName();
                request.setAttribute("INFOUSER", result);
            }else{
                request.setAttribute("INVALID", errorObject);
            }
        } catch (Exception e) {
            log("Error at SearchControllerUser: " + e.getMessage());
        }finally{
            request.getRequestDispatcher("searchforuser.jsp").forward(request, response);
        }
    }
